...
- **I1 violate 必拦**：执行路由是代码硬分支——只有 `verdict=allow` 或 `unclear_resolve.resolution=allow_once` 两个入口能进入执行；裁决 JSON 中不存在任何可触发执行的字段，模型无放行通道。violate 的请求同步返回停车场卡片，永不下发执行。
